...
def store(s: pywasm.Store, ptr: int, data: ctypes.Structure):
    ## convert any ctypes.Structure into actual bytes
    data = bytes(data)
    s.memory_list[0].data[ptr:ptr+len(data)] = data

def storestr(s: pywasm.Store, ptr: int, len: int, data: bytes):
    ## add a terminating NUL, but then truncate to len bytes
    data = (data + b'\x00')[:len]
    store(s, ptr, data)

def load(s: pywasm.Store, ptr: int, datatype):
    return datatype.from_buffer_copy(s.memory_list[0].data[ptr:])

def loadbytes(s: pywasm.Store, ptr: int, len: int):
...
def wasi_oflags_to_flags(oflags):
    flags = 0
...
class Wasi:
    def __init__(self, args: typing.List[str] = [], verbose: bool = False, rootdir: str = None):
        self.fds = {
    ...
    def trace(self, op, *args):
        if self.verbose:
            print(op, *args)

    def unimpl(self, name):
        def do(s: pywasm.Store, *args):
    ...
    def fd_readdir(self, s: pywasm.Store, fd: int, buf: int, buflen: int, cookie: int, retsize: int) -> int:
        ...
        names = [n.encode('utf-8', 'ignore') for n in names]
        dirent = dirent_t()
        cc = 0
        while buflen >= cc + ctypes.sizeof(dirent):
            ...
            dirent.ino = 0
            dirent.namelen = len(names[cookie])
            dirent.type = WASI_FILETYPE_UNKNOWN
            store(s, buf+cc, dirent)
            cc += ctypes.sizeof(dirent)
            namelen = min(buflen-cc, len(names[cookie]))
            store(s, buf+cc, names[cookie][:namelen])
            cc += namelen
            cookie += 1
    ...
    def proc_exit(self, s: pywasm.Store, rval: int):
        exit(rval)
